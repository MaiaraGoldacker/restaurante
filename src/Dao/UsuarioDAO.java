...
public class UsuarioDAO {
    ...
    public Usuario findByUser(String dsUsuario) throws SQLException {
        Usuario usuario = new Usuario();

        PreparedStatement stmt = Utilidades.getInstance().pegarConexaoBD().prepareStatement("SELECT ID FROM USUARIO WHERE IEATIVO = 1 AND DSUSUARIO = ?");
        stmt.setString(1, dsUsuario);
        ResultSet rs = stmt.executeQuery();

        if (rs.next()){
             usuario = getById(rs.getInt("ID"));
        }
        Utilidades.getInstance().pegarConexaoBD().close();
        return usuario;
    }
